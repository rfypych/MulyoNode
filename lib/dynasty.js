...
async function mulaiProses(scriptPath, options) {
  const { delayBlusukan, isAnakEmas: _isAnakEmas, toleransiKritik, koalisiGemuk } = options;
  ...
  try {
    ...
    anakEmasProses.on('exit', async (code, _signal) => {
      if (code !== 0) {
        jumlahBlusukan++;
 ...
 */
function simulasiBansos(size, _target) {
  // Parse size
  let bytes = 0;
