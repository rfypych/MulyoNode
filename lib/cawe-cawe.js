...
function intervensiGlobal() {
    ...
    // ========================================
    process.on('unhandledRejection', (alasan, _janji) => {
        janjiTakDitepati++;
...
function pasangPengawasan(options = {}) {
    const {
        ...
        promosiPrestasi: _promosiPrestasi = true  // Tambahkan prestasi di output
    } = options;

    // Simpan console.log asli
    const logAsli = console.log.bind(console);
    const _errorAsli = console.error.bind(console);
